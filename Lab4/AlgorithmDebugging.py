...
msg25_ON = mido.Message('note_on', note=25)
msg25_OFF = mido.Message('note_off', note=25)
msg26_ON = mido.Message('note_on', note=26)
msg26_OFF = mido.Message('note_off', note=26)
msg27_ON = mido.Message('note_on', note=27)
msg27_OFF = mido.Message('note_off', note=27)
msg28_ON = mido.Message('note_on', note=28)
msg28_OFF = mido.Message('note_off', note=28)
msg29_ON = mido.Message('note_on', note=29)
msg29_OFF = mido.Message('note_off', note=29)
msg30_ON = mido.Message('note_on', note=30)
msg30_OFF = mido.Message('note_off', note=30)
msg31_ON = mido.Message('note_on', note=31)
msg31_OFF = mido.Message('note_off', note=31)
msg32_ON = mido.Message('note_on', note=32)
msg32_OFF = mido.Message('note_off', note=32)
msg33_ON = mido.Message('note_on', note=33)
msg33_OFF = mido.Message('note_off', note=33)
msg34_ON = mido.Message('note_on', note=34)
msg34_OFF = mido.Message('note_off', note=34)
msg35_ON = mido.Message('note_on', note=35)
msg35_OFF = mido.Message('note_off', note=35)
...
###############################################################################
#Define how to move between positions#
###############################################################################
#Electric keyboard broken into 9 sections

#<-backward#
#LS0, LS1, LS2, LS3, RS4, RS3, RS2, RS1, RS0
#forward ->#


def Left_forward():
	if LX = 3 
		forward(int(100)/1000.0, int(50)) #100/1000 = speed, 50 = pos1
	elif LX = 2
		forward(int(100)/1000.0, int(100))
	elif LX = 1 
		forward(int(100)/1000.0, int(150))
		
def Left_backward():
	if LX = 3 
		backwards(int(100)/1000.0, int(50))
	elif LX = 2
		backwards(int(100)/1000.0, int(100))
	elif LX = 1 
		backwards(int(100)/1000.0, int(150))

def Right_forward():
	if RX = 4
		forward(int(100)/1000.0, int(50))
	elif RX = 3 
		forward(int(100)/1000.0, int(100))
	elif RX = 2
		forward(int(100)/1000.0, int(150))
	elif RX = 1 
		forward(int(100)/1000.0, int(200))
...
def Right_backward():
	if RX = 4
		...
		backwards(int(100)/1000.0, int(200))
###############################################################################
...
#Jason - need to make this probably user input or selectable or else you'll just have multiple scripts with only the file name changing.
	if LS0 < LS1 < LS2 <LS3 and RS0 < RS1 < RS2 < RS3 < RS4: #Prob useless
